...
export default function DeleteComerico() { // Define un componente funcional llamado DeleteComerico
    ...
    const handleSubmit = async (event) => { // Define una función asincrónica llamada handleSubmit que se ejecuta al enviar el formulario
        ...
        try {
            ...
            const data = response.json() // Convierte la respuesta en formato JSON
    ...
    return (
        <> {/* Fragmento de React */}
            <div className="eliminar-comercio-container"> {/* Contenedor principal con una clase CSS */}
                ...
                <button onClick={() => router.push('/options/comercio')} className="eliminar-comercio-button">Volver atras</button> {/* Botón para navegar a otra página */}
            </div>
        </>
    )
}